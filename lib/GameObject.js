...
export default class GameObject extends DrawingObject {
    ...
    checkCollisionWith(other) {
        for (const subjectCollider of this.colliders) {
            for (const obstacleCollider of other.colliders) {
                if (CollisionUtils.checkCollision(subjectCollider, this.globalTransform, obstacleCollider, other.globalTransform)) {
                    return true;
                }
            }
        }
    ...
    checkCollisionWithPoint(x, y) {
        for (const collider of this.colliders) {
            if (CollisionUtils.checkPointInCollider(x, y, collider, this.globalTransform)) {
                return true;
            }
        }
        return false;
    ...
    drawColliders(color) {
        ...
        for (const collider of this.colliders) {
            ...
            if (collider.type === "rectangle") {
                graphics.rect(collider.x - collider.width / 2, collider.y - collider.height / 2, collider.width, collider.height);
            }
            else if (collider.type === "ellipse") {
                graphics.ellipse(collider.x, collider.y, collider.width / 2, collider.height / 2);
            }
